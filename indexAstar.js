...
bluefood.push(new Food(25, 6, "blue"));
  ...
  for (var i = 0; i < maze.length; i++) {
    for (var j = 0; j < maze[i].length; j++) {
      if (maze[i][j] == 1) {
        ...
        var text =
          Math.round(getManhattan(redpac[0].x, redpac[0].y, i, j)) +
          Math.round(getManhattan(i, j, bluefood[0].x, bluefood[0].y));

        ctx.fillText(
          text,
          (MARGIN + WIDTH) * i + MARGIN,
          (MARGIN + WIDTH) * j + MARGIN + 25
        );
      }
    }
  }
...
function AggressiveRed() {
  redpac.map(function(p) {
    ...
    if (lis.length > 0) {
      ...
      var start = graph.nodes[p.x][p.y];
      var end = graph.nodes[temp.f.x][temp.f.y];
      p.stack = colorastar.search(graph.nodes, start, end);
    }
    move(p);
  });
}
