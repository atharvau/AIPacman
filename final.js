...
function draw() {
  ...
  if (bluefood.length > 5) AggressiveAvoidQTest();
  else DefensiveRedPatrol(redpac[0]);
  AggressiveBlue();

  DefensiveRedPatrol(redpac[1]);
  DefensiveBluePatrol();
  ...
  if (timer > 250) {
    ...
    ctx.fillText(
      "Red Team Wins",
      (MARGIN + WIDTH) * 15 + MARGIN,
      (MARGIN + WIDTH) * 18 + MARGIN
    );
  }
  if (count === 0) {
    redpac[0].shuffle();
...
function gameRunner() {
  ...
  try {
    if (travx[0][0] === redpac[0].x && travx[0][1] === redpac[0].y) {
      travx = removeFirst(travx);
    }
  } catch (error) {}

  console.log(redpac[0]);
}
...
function DefensiveRedPatrol(p) {
  try {
    ...
    if (p.nearEnemy[0].dist < 6) {
      ...
      if (p.x === 12 && p.y === 4) {
        p.patrolr = [12, 7];
      } else if (p.x === 12 && p.y === 7) {
        p.patrolr = [12, 4];
      }
      // console.log(p.patrol);
      var end = graph.nodes[p.patrolr[0]][p.patrolr[1]];
      p.stack = astar.search(graph.nodes, start, end);
    }
    // }
    move(p);
...
function pre() {
  ...
  redtoDefend(redpac[1]);

  ////////////////////////////////////////

  bluetoDefend(bluepac[0]);
  bluetoDefend(bluepac[1]);
  //////////////////////////////////////////////
  redToEnemy(redpac[0]);
  redToEnemy(redpac[1]);
  ////////////////////////////////////////////
  blueToEnemy(bluepac[0]);
  blueToEnemy(bluepac[1]);
...
function qlearningInit() {
  ...
  // }
  // // qVal = list2;
  // qVal = JSON.parse(window.localStorage.getItem("qVal"));
  // for (var i = 0; i < 36; i++) {
  //   var list = [];
  //   for (var j = 0; j < 14; j++) {
  //     console.log(
  //       Math.max(qVal[i][j].u, qVal[i][j].d, qVal[i][j].l, qVal[i][j].r)
  //     );
  //   }
  //   console.log("\n");
  // }
